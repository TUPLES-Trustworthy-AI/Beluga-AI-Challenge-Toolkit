...
class BelugaProblem:
    ...
    def __init__(self):
        ...
        self.tt_last : None | list[tuple[str]] = None
        self.tt_next : None | list[str] = None
        self.tt_prob : None | list[float] = None

    def get_name_lower_case_mapping(self): 
        mapping: dict[str,str] = {}
        for name in [o.name for o in self.trailers_beluga + 
                     self.trailers_factory + self.hangars + self.racks +
                     self.production_lines + self.production_lines]:
        ...
        if isinstance(obj, BelugaProblem):
            res = {
                'trailers_beluga': [{
                        'name': t.name,
                        'jig': t.jig
                    } for t in obj.trailers_beluga],
                'trailers_factory': [{
                        'name': t.name,
                        'jig': t.jig
                    } for t in obj.trailers_factory],
                'hangars': [{
                        'name': h.name,
                        'jig': h.jig
                    } for h in obj.hangars],
...
class BelugaProblemDecoder(json.JSONDecoder):
    ...
    def decode(self, json_str) -> BelugaProblem:
        ...
        beluga_problem.hangars = [Hangar(**h) for h in obj.get('hangars', [])]
