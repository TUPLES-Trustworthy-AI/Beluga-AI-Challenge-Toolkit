...
def encode(
    ...
    add_production_line_goals = True # indicated wether the goals thats the jigs must go to the production line (empty(j)) are added
) -> PDDLProblem:
    ...
    init_trailers(beluga_problem, state, problem, domain)
    ...
    def_goal(beluga_problem, problem, domain, add_production_line_goals)
...
def init_trailers(
    ...
    for trailer in beluga_problem.trailers_beluga:

        if state.trailer_load[trailer.name] == None and trailer.jig == None:
            problem.add_init(
                domain.get_predicate("empty").inst(PDDLParam(trailer.name, trailer_t))
            )
        else:
            # print(trailer.jig, 'in trailer', trailer.name)
            jig = state.trailer_load[trailer.name] if state.trailer_load[trailer.name] != None else trailer.jig
            problem.add_init(
    ...
    for trailer in beluga_problem.trailers_factory:
...
def init_hangar(
    ...
    for h in beluga_problem.hangars:
        problem.add_object(PDDLParam(h.name, hangar_t))

    problem.add_init(PDDLComment("hangars:"))
    for h in beluga_problem.hangars:
        if state.hangar_host[h.name] == None and h.jig is None:
            problem.add_init(domain.get_predicate("empty").inst(PDDLParam(h.name, hangar_t)))
        else:
            jig = state.hangar_host[h] if state.hangar_host[h.name] != None else h.jig
            problem.add_init(domain.get_predicate("in").inst(
                PDDLParam(jig,jig_t),
                PDDLParam(h.name, hangar_t)) # TODO changed by Michele
            )
...
def def_goal(beluga_problem: BelugaProblem, problem: PDDLProblem, domain: PDDLDomain, add_production_line_goals: bool):
    problem.add_goal(
        PDDLComment("All jigs empty (order defined by production schedule)")
    )

    if add_production_line_goals:
        for pl in beluga_problem.production_lines:
